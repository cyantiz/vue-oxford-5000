...
<template>
	<div class="instruction-btn bg-gray-400 hover:bg-white transition-all fixed left-3 top-3 z-10 w-6 h-6 cursor-pointer"
	@click="showInstruction = true">
	</div>
	<div class="settings-btn bg-gray-400 hover:bg-white transition-all fixed z-10 top-3 left-12 w-6 h-6 cursor-pointer"
	@click="showSettings = true">
	</div>
	<Instruction v-show="showInstruction" @close="showInstruction = false" />
	<Settings v-if="showSettings" :current-filter="filter" @close="showSettings = false" @save="(e) => saveSettings(e)" />
	<WordFlashCard :words="filteredWords" />
</template>
